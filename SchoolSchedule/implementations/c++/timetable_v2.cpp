...
struct state{
	...
	int penalty(bool flag = false){
		...
		int penalty = (penaltyA + penaltyB) * 100 + holes(flag);
...
int main(){
	...
	while(1){
		...
		for(int l = 0; l < 30; l++) for(int l1 = l + 1; l1 < 30; l1++){
			...
			if(S1.tryToChangeLesson(l / 6, l % 6, l1 / 6, l1 % 6) && S1.penalty() < penalty){
		...
		for(int c = 0; c < m; c++){
			for(int l = 0; l < 30; l++) for(int l1 = l + 1; l1 < 30; l1++){
				state S1 = S;
				if(S1.tryToChangeClass(c, l / 6, l % 6, l1 / 6, l1 % 6) && S1.penalty() < penalty){
					imporved = true;
					minS = S1;
					penalty = S1.penalty();
				}
		...
		for(int t = 0; t < n; t++){
			for(int l = 0; l < 30; l++) for(int l1 = l + 1; l1 < 30; l1++){
				state S1 = S;
				if(S1.tryToChangeTeacher(t, l / 6, l % 6, l1 / 6, l1 % 6) && S1.penalty() < penalty){
					imporved = true;
					minS = S1;
					penalty = S1.penalty();
				}
