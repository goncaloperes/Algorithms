...
class state:
    ...
    def penalty(self, flag = False):
        
        penaltyA = 0
        penaltyB = 0
        T = self.T
        C = self.C
        penaltyA = ((T >= 0).sum(axis = 2) - 4).clip(min=0).sum() + ((C >= 0).sum(axis = 2) - 4).clip(min=0).sum()
        for t in range(n):
            for d in range(5):
                for i in range(5):
        ...
        penalty = (penaltyA + penaltyB) * 100 + self.holes(flag)
...
parser.add_argument('--data', metavar = ': input file', type = str,help = 'input file for this problem', default='I_new.txt')
...
while True:
    ...
    for c in range(m):
        for l in range(30):
            for l1 in range(l + 1, 30):
                S1 = S.copy()
                if S1.tryToChangeClass(c, int(l / 6), l % 6, int(l1 / 6), l1 % 6) and S1.penalty() < penalty:
                    penalty = S1.penalty()
                    improved = True
                    minS = S1.copy()
    for t in range(n):                    
        for l in range(30):
            for l1 in range(l + 1, 30):
                S1 = S.copy()
                if S1.tryToChangeTeacher(t, int(l / 6), l % 6, int(l1 / 6), l1 % 6) and S1.penalty() < penalty:
                    penalty = S1.penalty()
                    improved = True
                    minS = S1.copy()
